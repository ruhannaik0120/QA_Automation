...
This contains the environment setup and verification scripts:
...
Their installation and verification procedures are documented in [SETUP_GUIDE.md](SETUP_GUIDE.md).
...
Complete installation, environment configuration, dependency verification, and initial MCP startup by following [SETUP_GUIDE.md](SETUP_GUIDE.md). Those procedures are maintained in one place and are not duplicated here.

## 9. AI-Client Integration Reference

The server uses MCP stdio transport and can be registered by any compatible AI client using the repository Python environment, `MCP/server.py`, and `MCP/` as the working directory. Client-specific registration steps and connection verification are centralized in [SETUP_GUIDE.md](SETUP_GUIDE.md).
...
Complete installation and MCP-client registration through [SETUP_GUIDE.md](SETUP_GUIDE.md). After the server is connected:
