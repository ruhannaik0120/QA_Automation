...
- Explicitly declare the workflow's permanent input-acquisition mode in this document using the following workflow-body contract; shared configuration does not choose a mode:
...
- For `manual`, define any required reference discovery, selection approval, user placement, local verification, durable state, and resume behavior.
- For `automatic`, explicitly permit the hardened downloader, define required acquisition approvals, require configured authentication, and preserve every download, archive, path, credential, size, rollback, overwrite, and manifest safety control.
- In either mode, a hyperlink is never proof that its content was acquired or inspected.
...
- Preserve this order when applicable: successful global preflight; workspace initialization; direct Jira retrieval; the explicitly declared manual or automatic acquisition path and its approvals; durable combined context and context approval; QA planning; safe database-profile discovery; presentation and approval of exact read-only SQL scope and profile mappings; approved profile switching and connection validation; one approved statement per MCP call; and final evidence, reports, and logs.
- Keep every acquisition approval required by the selected mode separate from context and database-execution approval. Configuration, authentication, connector, dependency, routing, and profile-discovery failures are blockers; DDL, DML, and environment changes require separate explicit authorization.
...
- Require the completed workflow to choose exactly one supported acquisition mode and define its artifacts, approvals, tools, safety boundaries, failure paths, and resume evidence without relying on a shared default.
- In manual mode, prohibit downloader invocation and define how approved local files are verified before inspection.
- In automatic mode, allow `modules.download_ticket_inputs` only when the workflow explicitly permits it, required approvals are satisfied, authentication is configured, and all hardened downloader controls remain active.
- Keep acquisition approval and combined-context approval separate whenever the selected mode defines an acquisition checkpoint; neither approval may imply the other.
